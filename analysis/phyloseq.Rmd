...
```{r}
save(ps_raw_bact, file = "../data/dada2/phyloseq/ps_raw_bact.RData")
save(ps_dec_bact, file = "../data/dada2/phyloseq/ps_dec_bact.RData")
```
...
Subset certain taxa of interest
```{r}
# taxa of interest
ps_bact_samp_sub = subset_taxa(ps_bact_samp, Order=="Coxiellales" | Order=="Mycoplasmataceae" | Genus=="Bartonella" | Order=="Francisellales" | Genus=="Borreliella" | Genus=="Leptospira" | Family=="Rickettsiaceae" | Family=="Anaplasmataceae" | Family=="Midichloriaceae" | Genus=="Rickettsiella")
ps_bact_samp_sub = prune_samples(sample_sums(ps_bact_samp_sub)>=1000, ps_bact_samp_sub)
ps_bact_samp_sub = filter_taxa(ps_bact_samp_sub, function(x) sum(x > 1) > (0.01*length(x)), TRUE)
```

Add tree for this taxa
```{r}
tree2 <- read_tree("data/dada2/top38.nwk")
ps_bact_samp_sub <- merge_phyloseq(ps_bact_samp_sub, tree)
```
...
#convert
bact_amp <- phyloseq_to_ampvis2(ps_dec_bact)
```

Subset ampvis2 object based on sample category
```{r}
#remove controls
amp_samp <- amp_subset_samples(bact_amp, 
